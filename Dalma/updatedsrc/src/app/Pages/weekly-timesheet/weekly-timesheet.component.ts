...
monthOptions: any[] = []; // {label, value} objects
filteredMonths: any[] = [];
...
onMonthSelect(event: any) {
  ...
  const monthIndex = this.months.indexOf(selectedMonth); // 0-based
  ...
  this.timesheetRows = this.allTimesheetRows.filter(row => {
    ...
    try {
      // Attempt to parse any string as a Date
      ...
      return rowDate.getMonth() === monthIndex;
    } catch (error) {
  ...
  console.log('Filtered rows:', this.timesheetRows);
}
...
private loadExistingLeaveCounts() {
  this.apiService.GetAllTimesheetLeave().subscribe({
    next: (leaveData: any) => {
      if (!Array.isArray(leaveData) || leaveData.length === 0) return;
      const userLeaveData = leaveData.filter((leave: any) => {
        ...
        return leaveEmployeeName.includes(this.currentUserName.toLowerCase());
      });
...
if (row[leaveLockedProp] && !this.isAdmin && !this.isManager) {
  // Do not show dropdown if day is locked and the current user is not admin/manager
  row[`show${this.capitalizeFirstLetter(day)}LeaveDropdown`] = false;
  this.messageService.add({
    severity: 'warn',
    summary: 'Leave Locked',
    detail: `${this.getFullDayName(day)} has been submitted and locked. Please contact admin to edit.`,
    life: 4000
  });
  return;
}
 ...
 onLeaveTypeChange(rowIndex: number, day: string, leaveType: string, event?: Event) {
  ...
  // Prevent changing locked leave types for non-admins/managers
  if (row[leaveLockedProp] && !this.isAdmin && !this.isManager) {
    this.messageService.add({
      severity: 'warn',
      summary: 'Cannot edit leave',
      detail: `${this.getFullDayName(day)}: leave type already submitted and cannot be edited. Please contact admin to edit.`,
      life: 3000
    });
      ...
      if (Array.isArray(leaveData) && leaveData.length > 0) {
        ...
        this.timesheetRows.forEach(row => {
          ...
          weekLeaves.forEach((leave: any) => {
            ...
            // If there's no leave type (blank), clear UI and lock for non-admins
            if (!leaveType || leaveType === '') {
              row[leaveTypeProperty] = '';
              row[showDropdownProperty] = false;
              row[leaveLockedProp] = !this.isAdmin; // lock for non-admins, allow admin to edit
              return;
            }
            ...
            if (this.isAdmin || this.isManager) {
              if (
                ...
                row[showDropdownProperty] = true;
                row[leaveLockedProp] = !this.isAdmin;
              }
            } else {
              // Non-admin (normal user): apply leave directly
              row[leaveTypeProperty] = leaveType;
              row[showDropdownProperty] = true;
              row[leaveLockedProp] = !this.isAdmin;
            }
...
private DAYS: string[] = ['sun','mon','tues','wed','thurs','fri','sat'];

private rememberOriginalLeaveTypes(): void {
  if (!Array.isArray(this.timesheetRows)) return;
  this.timesheetRows.forEach(row => {
    this.DAYS.forEach(d => {
      // store original value (string) for later comparison
      row[`${d}OriginalLeaveType`] = (row[`${d}LeaveType`] || '').toString();
    });
  });
}

// Put this inside your WeeklyTimesheetComponent class
private validateLockedLeaveChanges(rowsToSave: any[]): { isValid: boolean; message?: string } {
  // Compare each row that will be saved with the original snapshot
  // originalTimesheetRows should be a snapshot that contains the original per-day leave types
  for (const rowToSave of rowsToSave) {
    // try to find matching original row by timesheetId or by weekNo+project+employee
    const originalRow = this.originalTimesheetRows.find(orig =>
    ...
    // if we don't have an originalRow, nothing to lock against
    if (!originalRow) continue;

    const days = ['sun', 'mon', 'tues', 'wed', 'thurs', 'fri', 'sat'];
    for (const d of days) {
      const origLeave = (originalRow[`${d}LeaveType`] || originalRow[`${d}OriginalLeaveType`] || '').toString();
      const newLeave = (rowToSave[`${d}LeaveType`] || '').toString();
      const lockedFlag = !!(originalRow[`${d}LeaveLocked`] || rowToSave[`${d}LeaveLocked`]);

      // If original had a value AND lockedFlag is true AND changed by non-admin/manager -> block
      if (lockedFlag && origLeave !== '' && origLeave !== newLeave && !this.isAdmin && !this.isManager) {
        const dayName = this.getFullDayName(d);
        const empName = rowToSave.employeeName || this.currentUserName || 'Employee';
         error: (err: any) => {
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: err.error?.message || 'Leave not editable please connect to admin'
      });
    }
      }
    }
