...
resetModal() {
  ...
  paidLeaveAllocated : number = 0;
  casualLeaveAllocated : number = 0;
  flexHolidayAllocated: number = 0;
...
onMonthSelect(event: any) {
  ...
  if (selectedRow) {
    selectedRow.showSunLeaveDropdown = false;
    selectedRow.showMonLeaveDropdown = false;
    selectedRow.showTuesLeaveDropdown = false;
    selectedRow.showWedLeaveDropdown = false;
    selectedRow.showThursLeaveDropdown = false;
    selectedRow.showFriLeaveDropdown = false;
    selectedRow.showSatLeaveDropdown = false;
    
    const days = ['sun', 'mon', 'tues', 'wed', 'thurs', 'fri', 'sat'];
    days.forEach(day => {
      const hours = selectedRow[`${day}Hours`] || 0;
      const leaveType = selectedRow[`${day}LeaveType`];
      
      if (hours === 0 && leaveType && leaveType.trim() !== '') {
        selectedRow[`show${this.capitalizeFirstLetter(day)}LeaveDropdown`] = true;
      }
    });
  }
  ...
  async getEmployeeJoinDate() {
    try {
      ...
      const currentUser = this.selectedEmployee || users.find((u: any) =>
        u.email === this.currentUserEmail ||
        `${u.firstName} ${u.lastName}`.trim() === this.currentUserName
      );
      console.log(currentUser);
     
      if (currentUser && currentUser.joiningDate) {
        this.employeeJoinDate = new Date(currentUser.joiningDate);
        // Update current user details from API response
        this.currentUserName = currentUser.employeeName || `${currentUser.firstName} ${currentUser.lastName}`.trim();
        this.currentUserId = currentUser.empId || currentUser.EmpId || this.currentUserId;
        //sessionStorage.setItem('joiningDate', currentUser.joiningDate);
        const monthName = this.employeeJoinDate.toLocaleDateString('en-US', { month: 'short' });
        this.apiService.getLeaveMonthData(monthName).subscribe({
          next: (data) => {
            this.leaveMonthData = data;
            console.log('Leave month data for joining month:', data);
            if (this.employeeJoinDate && data) {
              const joinDay = this.employeeJoinDate.getDate();
              const isBefore15 = joinDay <= 15;
              this.paidLeaveAllocated  = isBefore15 ? (data.paidLeaveBefore15 || 0) : (data.paidLeaveAfter15 || 0);
              this.casualLeaveAllocated = isBefore15 ? (data.sickLeaveCasualLeaveBefore15 || 0) : (data.sickLeaveCasualLeaveAfter15 || 0);
              this.flexHolidayAllocated = data.flexHoliday || 0;
              this.totalAllocated = this.paidLeaveAllocated + this.casualLeaveAllocated;
              console.log('Entitlements set:', {
                paidLeave: this.paidLeaveAllocated,
                casualLeave: this.casualLeaveAllocated,
                flexHoliday: this.flexHolidayAllocated,
                isBefore15
              });
              // Update allocated values in leaveTypeCounts
              this.leaveTypeCounts.PaidLeave.allocated = this.paidLeaveAllocated;
              console.log('Updated Paid Leave Allocated:', this.leaveTypeCounts.PaidLeave.allocated);
             
              this.leaveTypeCounts.CasualLeaveCount.allocated = this.casualLeaveAllocated;
              this.leaveTypeCounts.FlexiHoliday.allocated = this.flexHolidayAllocated;
              this.leaveTypeCounts.Total.allocated = this.paidLeaveAllocated + this.casualLeaveAllocated;
            } else {
              console.warn('No data received or employee join date not set');
            }
          },
          error: (err) => {
            console.error('Error fetching leave month data:', err);
          }
        });
 ...
 private applyProRataRounding(value: number, roundUp: boolean): number {
    if (roundUp) {
        return Math.ceil(value * 2) / 2; // round up to nearest 0.5
    } else {
        return Math.floor(value * 2) / 2; // round down to nearest 0.5
    }
}
  ...
  this.apiService.GetAllTimesheetLeave().subscribe({
    next: (leaveData: any) => {
      if (!Array.isArray(leaveData) || leaveData.length === 0) {
        ...
        this.calculateLeavesTakenWithTakenValue([]);
      ...
      this.calculateLeavesTakenWithTakenValue(userLeaveData);
      this.updateRemainingLeaves();
    },
    error: (err) => {
...
private calculateLeavesTakenWithTakenValue(leaveData: any[]) {
  this.leaveTracking.clear();
  const uniqueLeaves = new Map<string, number>();
  console.log('All leave records from DB:', leaveData.map(l => ({
    type: l.leaveType,
    taken: l.taken || l.Taken,
    day: l.day,
    week: l.fiscalWeek || l.FiscalWeek,
    empId: l.empId || l.EmpId
  })));

  leaveData.forEach((leave: any) => {
    const leaveType = leave.leaveType || leave.LeaveType;
    
    // Skip if no leave type or empty
    if (!leaveType || leaveType === 'SWITCH_TO_HOURS' || leaveType.trim() === '') {
      return;
    }

    // Get Taken value from database
    const takenValue = parseFloat(leave.taken || leave.Taken || 1);
    
    // Create a unique key: employee + week + day + leaveType
    // This prevents same day from being counted multiple times
    const employeeId = (leave.empId || leave.EmpId || '').toString().trim();
    const fiscalWeek = (leave.fiscalWeek || leave.FiscalWeek || '').toString().trim();
    const day = (leave.day || leave.Day || '').toString().trim();
    
    const uniqueKey = `${employeeId}-${fiscalWeek}-${day}-${leaveType}`;
    
    console.log(`Processing: ${uniqueKey} - Taken: ${takenValue}`);
    
    // If this unique key already exists, keep the HIGHEST taken value
    // (if someone updates from half-day to full-day, count full-day)
    if (uniqueLeaves.has(uniqueKey)) {
      const existingValue = uniqueLeaves.get(uniqueKey) || 0;
      if (takenValue > existingValue) {
        uniqueLeaves.set(uniqueKey, takenValue);
        console.log(`Updated duplicate (higher value): ${uniqueKey} from ${existingValue} to ${takenValue}`);
      } else {
        console.log(`Skipping duplicate (lower or equal value): ${uniqueKey}`);
      }
    } else {
      uniqueLeaves.set(uniqueKey, takenValue);
    }
  ...
  uniqueLeaves.forEach((takenValue, uniqueKey) => {
    // Extract leave type from unique key
    const parts = uniqueKey.split('-');
    const leaveType = parts[parts.length - 1]; // Last part is leaveType
    
    const currentCount = this.leaveTracking.get(leaveType) || 0;
    this.leaveTracking.set(leaveType, currentCount + takenValue);
    
    console.log(`Counting: ${leaveType} - Taken: ${takenValue} (unique)`);
  });

  console.log('Final leaves taken (deduplicated):', Object.fromEntries(this.leaveTracking));
}
...
private updateRemainingLeaves() {
  this.leaveEntitlements.forEach(entitlement => {
    ...
    // Calculate remaining with decimal precision
    entitlement.remaining = Math.max(0, entitlement.proRataEntitlement - taken);
    
    console.log(`Update: ${entitlement.leaveType} - 
      ProRata: ${entitlement.proRataEntitlement}, 
      Taken (deduplicated): ${taken}, 
  ...
  private processTimesheets(allTimesheets: any[], filteredTimesheets: any[]) {
    ...
    sunDescription: item.sunDescription ?? item.SunDescription ?? '',
    monDescription: item.monDescription ?? item.MonDescription ?? '',
    tuesDescription: item.tuesDescription ?? item.TuesDescription ?? '',
    wedDescription: item.wedDescription ?? item.WedDescription ?? '',
    thursDescription: item.thursDescription ?? item.ThursDescription ?? '',
    friDescription: item.friDescription ?? item.FriDescription ?? '',
    satDescription: item.satDescription ?? item.SatDescription ?? '',
    // ✅ ✅ ✅ END FIX
    ...
    next: (existingLeaves: any[]) => {
      timesheetRows.forEach(row => {
        ...
        days.forEach(day => {
          ...
          const existingLeave = existingLeaves.find((leave: any) => {
            ...
            const leaveFiscalWeek = (leave.fiscalWeek || leave.FiscalWeek || '').toString().trim();
            const leaveDay = (leave.day || leave.Day || '').toString().trim();
            const leaveLeaveType = (leave.leaveType || leave.LeaveType || '').toString().trim();

            return leaveEmployeeName === currentUserName &&
                   leaveFiscalWeek === row.fiscalWeek &&
                   leaveDay === this.getFullDayName(day) &&
                   leaveLeaveType === leaveType;
          });
          if (existingLeave && leaveType && leaveType !== 'SWITCH_TO_HOURS') {
            const current = newLeavesByType.get(leaveType) || 0;
            newLeavesByType.set(leaveType, current + 1);
            return;
          }
          // CASE: Non-admin with existing leave but user entered hours -> treat as hours with half-day leave
          if (leaveType && hoursValue > 0 && nonAdmin) {
            const previousLeave = leaveType;
            
            const isHalfDay = hoursValue === 4;
            
            const taken = this.leaveTracking.get(previousLeave) || 0;
            
            if (isHalfDay) {
              this.leaveTracking.set(previousLeave, Math.max(0, taken - 0.5));

              const ent = this.leaveEntitlements.find(e => e.leaveType === previousLeave);
              if (ent) {
                ent.taken = Math.max(0, ent.taken - 0.5);
                ent.remaining = Math.max(0, ent.proRataEntitlement - ent.taken);
              }

              // Calculate remaining and ROUND to integer
              const remainingDecimal = this.calculateRemainingLeave(leaveType, newLeavesByType);
              const remainingInt = Math.round(remainingDecimal);
              
              leaveRecords.push({
                EmployeeName: this.currentUserName,
                LeaveType: previousLeave,
                FiscalWeek: row.fiscalWeek,
                Day: this.getFullDayName(day),
                RemainingLeave: remainingDecimal,
              ...
              const current = newLeavesByType.get(previousLeave) || 0;
              newLeavesByType.set(previousLeave, current + 0.5);
          ...
          // CASE 1: LEAVE SELECTED (Full day leave) - Only if NEW
          if (leaveType && leaveType !== 'SWITCH_TO_HOURS' && !existingLeave) {
            const current = newLeavesByType.get(leaveType) || 0;
            newLeavesByType.set(leaveType, current + 1);
          ...
          if (leaveType === 'SWITCH_TO_HOURS') {
            ...
            const wasHalfDay = row[`${day}WasHalfDay`] || false;
          ...
          // CASE 3: NO LEAVE TYPE (Regular hours)
      ...
      if (this.halfDayNames.length > 0) {
        this.halfDayNames.forEach(day => {
          let leaveTypeForHalfDay = 'Casual Leave';
          const sickEntitlement = this.leaveEntitlements.find(e => e.leaveType === 'Casual Leave');
          
          if (sickEntitlement) {
            const alreadyTaken = this.leaveTracking.get('Casual Leave') || 0;
            const newlyBeingTaken = newLeavesByType.get('Casual Leave') || 0;
         ...
         const remaining = this.calculateRemainingLeave(leaveTypeForHalfDay, newLeavesByType);
      ...
      // SAVE ONLY NEW RECORDS (skip duplicates)
      if (leaveRecords.length > 0) {
        leaveRecords.forEach(record => {
          this.apiService.saveTimesheetLeave(record).subscribe({
            next: () => console.log("Saved NEW leave record:", record),
            error: err => console.error("Error saving leave:", err)
          });
        });
        this.logCurrentLeaveStatus();
      } else {
        console.log("No new leave records to save (all are duplicates)");
      }
    },
    error: (err) => {
      console.error("Error fetching existing leaves:", err);
    }
  });
}
       ...
       if ((hours === 0 || hours === null) && !leaveType) {
        ...
        if (leaveType === 'Worked on another project') {
          ...
          if (!hasOtherProjectRow) {
            this.messageService.add({
             severity: 'warn',
              summary: 'Work On Another Project',
              detail: `${this.getFullDayName(day)} (Week ${row.weekNo}): You selected 'Worked on another project'. Please add a new row for other project hours.`,
              life: 10000
            });
          }
...
onHoursChanged(rowIndex: number, day: string) {
  ...
  // Check if hours changed from 4 to 8 and there is leave type
  const originalRow = this.originalTimesheetRows[rowIndex];
  if (originalRow && leaveType) {
    const originalHours = Number(originalRow[hoursProp]) || 0;
    if (originalHours === 4 && hoursValue === 8) {
      // Remove the half day leave
      const weekData = this.fiscalWeeks.find(w => w.weekNo === row.weekNo);
      if (weekData) {
        const startDate = new Date(weekData.startDate);
        const dayIndex = ['sun', 'mon', 'tues', 'wed', 'thurs', 'fri', 'sat'].indexOf(day);
        const leaveDate = new Date(startDate);
        leaveDate.setDate(startDate.getDate() + dayIndex);
        const entitlement = this.leaveEntitlements.find(le => le.leaveType === leaveType);
        const alreadyTaken = this.leaveTracking.get(leaveType) || 0;
        const remaining = entitlement ? entitlement.proRataEntitlement - alreadyTaken + 0.5 : 0;
        const record = {
          EmpId: row.EmpId,
          LeaveType: leaveType,
        ...
        this.apiService.saveTimesheetLeave(record).subscribe({
          next: () => {
            this.leaveTracking.set(leaveType, alreadyTaken - 0.5);
            this.updateRemainingLeaves();
            this.mapLeaveDataToCard();
            console.log('Half day leave removed for', day);
          },
          error: (err) => console.error('Error removing leave:', err)
        });
      }
    }
  }
...
openWorkDetails(rowIndex: number, day: string, event: MouseEvent): void {
  ...
  const descProp = `${day}Description`;

  console.log('OPEN MODAL CHECK →', {
    rowIndex,
    day,
    descProp,
    value: row[descProp],
...
// Added for 3 Dots
private normalizeDescriptions(row: any): any {
  return {
    ...row,
    sunDescription: row.sunDescription ?? row.SunDescription ?? '',
    monDescription: row.monDescription ?? row.MonDescription ?? '',
    tuesDescription: row.tuesDescription ?? row.TuesDescription ?? '',
    wedDescription: row.wedDescription ?? row.WedDescription ?? '',
    thursDescription: row.thursDescription ?? row.ThursDescription ?? '',
    friDescription: row.friDescription ?? row.FriDescription ?? '',
    satDescription: row.satDescription ?? row.SatDescription ?? ''
  };
}
