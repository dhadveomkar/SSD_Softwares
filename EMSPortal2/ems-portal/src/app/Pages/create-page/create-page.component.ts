...
;
  ...
  private loadManagers() {
    ...
    const handleUsers = (users: User[]) => {
      ...
      if (this.isEditMode && this.formData.reportingManager) {
        let currentManagerExists = this.managers.some(
          m => m.empId === this.formData.reportingManager
        );

        if (!currentManagerExists) {
          // First, try to find by name in case reportingManager is stored as name
          const managerByName = users.find(u => `${u.firstName} ${u.lastName}` === this.formData.reportingManager);
          if (managerByName) {
            this.managers.push(managerByName);
            this.formData.reportingManager = managerByName.empId;
            return;
          }

          // Then try to find by empId in the provided users list
          ...
          // As a fallback, fetch all users and try to locate the missing manager by empId
          this.apiService.getAllUsers().subscribe({
            next: (allUsers: User[]) => {
