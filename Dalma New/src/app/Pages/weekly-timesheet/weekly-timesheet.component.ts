...
applyProRataToEntitlements() {
  ...
  const joinDay = this.employeeJoinDate ? this.employeeJoinDate.getDate() : 1;
  const isJoinedBefore15th = joinDay <= 15;
  const joinMonthIndex = this.employeeJoinDate ? this.employeeJoinDate.getMonth() : 0;

  console.log('=== applyProRataToEntitlements() ===');
  console.log('Employee join date:', this.employeeJoinDate);
  console.log('Join month index:', joinMonthIndex);
  console.log('Is joined before 15th:', isJoinedBefore15th);
  console.log('Extracted entitlements:', entitlements);

  // Calculate months worked
  const calculateMonthsWorked = (joinMonth: number, isBefore15th: boolean): number => {
    if (isBefore15th) {
      return 12 - joinMonth; // Full months including join month
    } else {
      return Math.max(0, 11 - joinMonth); // Exclude join month
    }
  };

  const monthsWorked = calculateMonthsWorked(joinMonthIndex, isJoinedBefore15th);
  console.log('Months worked:', monthsWorked);

  // Pro-rata tables - ROUNDED TO WHOLE NUMBERS (not 2.5)
  const proRataBefore15: any = {
    12: 17, 11: 16, 10: 14, 9: 13, 8: 11,
    7: 10, 6: 8, 5: 7, 4: 5, 3: 4,
    2: 2, 1: 1, 0: 0
  };

  const proRataAfter15: any = {
    12: 16, 11: 15, 10: 13, 9: 12, 8: 10,
    7: 9, 6: 7, 5: 6, 4: 4, 3: 3,
    2: 1, 1: 0, 0: 0
  };

  this.leaveEntitlements = entitlements.map((entitlement: LeaveEntitlement) => {
    console.log(`\nProcessing entitlement: ${entitlement.type}`);
    
    // For Ennovate/Honeywell companies
    const companyName = sessionStorage.getItem('companyName') || '';
    const companyLower = companyName.toLowerCase();
    const leaveLower = entitlement.type.toLowerCase();
    
    let annualEntitlement = entitlement.annual;
    
    // Override based on company rules
    if ((companyLower.includes('ennovate') || companyLower.includes('honeywell'))) {
      if (leaveLower.includes('casual')) {
        annualEntitlement = 8;
      } else if (leaveLower.includes('paid')) {
        annualEntitlement = 17;
      } else if (leaveLower.includes('holiday') && !leaveLower.includes('optional')) {
        annualEntitlement = 8;
      } else if (leaveLower.includes('optional')) {
        annualEntitlement = 2;
      }
    }
    
    console.log(`Annual entitlement: ${annualEntitlement}`);

    // Calculate pro-rata value
    let proRataValue = 0;
    
    if (annualEntitlement === 17) {
      proRataValue = isJoinedBefore15th 
        ? (proRataBefore15[monthsWorked] || 0)
        : (proRataAfter15[monthsWorked] || 0);
    } else if (annualEntitlement === 8) {
      const ratio = annualEntitlement / 17;
      const paidProRata = isJoinedBefore15th 
        ? (proRataBefore15[monthsWorked] || 0)
        : (proRataAfter15[monthsWorked] || 0);
      proRataValue = paidProRata * ratio;
      // Round up to nearest whole number
      proRataValue = Math.ceil(proRataValue);
    } else if (annualEntitlement === 2) {
      const monthlyRate = annualEntitlement / 12;
      let rawValue = monthsWorked * monthlyRate;
      
      // Round up to nearest whole number
      proRataValue = Math.ceil(rawValue);
      
      proRataValue = Math.min(proRataValue, annualEntitlement);
    } else {
      const monthlyRate = annualEntitlement / 12;
      let rawValue = monthsWorked * monthlyRate;
      
      // Always round up for pro-rata calculations
      proRataValue = Math.ceil(rawValue);
      
      proRataValue = Math.min(proRataValue, annualEntitlement);
    }

    console.log(`Pro-rata value calculated: ${proRataValue}`);

    return {
      leaveType: entitlement.type,
      annualEntitlement: annualEntitlement,
      proRataEntitlement: proRataValue,
      remaining: proRataValue,
      taken: 0,
      proRataFactor: monthsWorked / 12,
      joinMonthIndex,
      monthsWorked: monthsWorked,
      roundingRule: 'Round Up to Whole Number'
    };
  });

  // Initialize leave tracking
  this.initializeLeaveTracking();
  
  // Store initial entitlements
  this.initialLeaveEntitlements = [...this.leaveEntitlements];
  
  console.log('\n=== Final Leave Entitlements ===');
  this.leaveEntitlements.forEach(e => {
    console.log(`${e.leaveType}: Annual=${e.annualEntitlement}, Pro-rata=${e.proRataEntitlement}, Taken=${e.taken}, Remaining=${e.remaining}`);
  });

  // Load existing leave counts
  this.loadExistingLeaveCounts();
}
...
private initializeLeaveTracking() {
  this.leaveTracking = new Map<string, number>();
  
  // Initialize tracking for all leave types
  this.leaveEntitlements.forEach(entitlement => {
    this.leaveTracking.set(entitlement.leaveType, 0);
  });
  
  console.log('Initialized leave tracking:', Object.fromEntries(this.leaveTracking));
}
...
// //   // ✔ Check only new rows user has filled
// //   const hours = [
...
//     // Call backend save, get flag if it existed
...
checkHalfDays() {
  ...
  days.forEach((day )=> {
    ...
    if (totalHoursForDay > 0 && totalHoursForDay < 6.5) {
      this.totalHalfDays++;
...
private proceedWithSaving()
 ...
 // NEW: Validate holiday consistency before saving
  ...
  private saveAllLeaveRecords(timesheetRows: any[]) {
    ...
    // First, count new leaves being added in this submission
    const newLeavesByType = new Map<string, number>();

    // Collect all leave records from all rows
    timesheetRows.forEach(row => {
      if (row.weekNo && row.fiscalWeek) {
        ...
        days.forEach(day => {
          ...
          // Only save if leave type is selected and not "SWITCH_TO_HOURS"
          if (leaveType && leaveType !== 'SWITCH_TO_HOURS') {
            // Count this leave for remaining calculation
            const currentCount = newLeavesByType.get(leaveType) || 0;
            newLeavesByType.set(leaveType, currentCount + 1);

            // Calculate remaining leave for this specific record
      ...
      const hours = row[hoursProperty];
       ...
       if ((hours === 0 || hours === null) && !leaveType) {
        ...
        if (leaveType === 'Worked on another project') {
          // Force hours to 0 for this project
          if (row[hoursProperty] === 0 || row[hoursProperty] === null) {
            row[hoursProperty] = 0;
          }
          // Look for another row with DIFFERENT project for the SAME employee and week
          ...
          if (!hasOtherProjectRow) {
            ...
            this.messageService.add({
              ...
              detail: `${this.getFullDayName(day)} (Week ${row.weekNo}): You selected "Worked on another project". Consider adding another row for the other project.`,
              life: 4000
            });
          }
        }
